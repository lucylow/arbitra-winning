...
persistent actor AIAnalysis {
    // Evidence Manager service interface
    type EvidenceManager = actor {
        getDisputeEvidence: (disputeId: Text) -> async [Types.Evidence];
    };
    
    // Stable variable to store evidence manager ID
    private stable var evidenceManagerId: ?Principal = null;
    
    // Get evidence manager actor
    private func getEvidenceManager() : EvidenceManager {
        switch (evidenceManagerId) {
            case (?id) {
                actor (Principal.toText(id)) : EvidenceManager;
            };
            case null {
                // Use default ID for local dev (will be set during initialization)
                // This should match the evidence_manager canister ID from dfx
                actor ("ulvla-h7777-77774-qaacq-cai") : EvidenceManager;
            };
        };
    };
    
    // Set evidence manager ID (should be called during initialization)
    public func setEvidenceManagerId(id: Principal) : async () {
        evidenceManagerId := ?id;
    };
    
    // Helper: Convert EvidenceType to Text
    private func evidenceTypeToText(et: Types.EvidenceType) : Text {
        switch (et) {
    ...
    public func analyzeDispute(
        ...
        // 1. Fetch all evidence from the Evidence Manager canister
        let evidenceManagerActor = getEvidenceManager();
        let evidenceResult = await evidenceManagerActor.getDisputeEvidence(disputeId);
        let evidenceDescriptions = Array.map<Types.Evidence, Text>(evidenceResult, func(e) {
            e.description # " (Type: " # evidenceTypeToText(e.evidenceType) # ")"
        });
        
        // 2. Simulate AI analysis with chain-of-thought reasoning
    ...
    public func reanalyzeDispute(
        ...
        await analyzeDispute(disputeId, disputeTitle, disputeDescription)
    };

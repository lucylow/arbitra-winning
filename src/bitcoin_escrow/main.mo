...
persistent actor BitcoinEscrow {
    private stable var admin: Principal = Principal.fromText("2vxsx-fae"); // Placeholder for the main Arbitra backend canister
    
    private func requireAdmin(caller: Principal) : Result.Result<(), Text> {
        if (caller == admin) {
            #ok(())
        } else {
            #err("Unauthorized: Only the Admin canister can perform this action.")
        }
    };
    // State variables

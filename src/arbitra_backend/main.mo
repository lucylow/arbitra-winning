...
persistent actor ArbitraBackend {
    private stable var admin: Principal = Principal.fromText("2vxsx-fae"); // Default to anonymous for local dev, should be set on mainnet
    
    private func requireAdmin(caller: Principal) : Result.Result<(), Text> {
        if (caller == admin) {
            #ok(())
        } else {
            #err("Unauthorized: Only the Admin can perform this action.")
        }
    };
    // State variables
